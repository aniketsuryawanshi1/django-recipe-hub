...
class Recipe(models.Model):
    ...
    def clean(self):
        super().clean()
        if self.author and hasattr(self.author, 'role') and self.author.role != 'seller':
    ...
    def increment_view_count(self):
        try:
            Recipe.objects.filter(pk=self.pk).update(view_count=models.F('view_count') + 1)
...
class RecipeView(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, related_name='recipe_views'
    )
